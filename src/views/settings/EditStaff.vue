    ...
    <div class="panel" >
      ...
      <div class="content">
        ...
        <el-form :model="editStaffForm" :rules="rules" ref="form" label-width="105px" :hide-required-asterisk='true'>
          ...
          <el-form-item >
            <span slot="label">管辖电梯1：<div class="subTip" >（第一负责人）</div></span>
            <el-dropdown trigger="click"  :hide-on-click="false" class="selectLiftDropdown">
              ...
              <el-dropdown-menu slot="dropdown" class="liftDropdown">
                <!-- 搜索栏 -->
                <div style="height:25px;padding-right: 10px;margin-bottom: 10px;">
                  <search-input v-model.trim="searchKey" placeholderValue="搜索电梯注册代码" @search="searchLift" @cancel="clearCode"></search-input>
                </div>
...
export default {
  data() {
    return {
      ...
      checkedLiftAs:[],
      checkedLiftBs:[],
      getLIftDataFirstOrigin: [],
      getLIftDataSecondOrigin: [],
  ...
  methods: {
          ...
          api.accountApi.getElevatorByArea(this.getElevatorByAreaParams1).then((res) => {
            if(res.data.code === 200 && res.data.message === 'success'){
                ...
                if(res.data.data.major.length > 0){
                  this.totalPageSize1 = res.data.data.majorTotal
                ...
                this.getLIftDataFirstOrigin = this.mergeArrayList(firstList)
          ...
          api.accountApi.getElevatorByArea(this.getElevatorByAreaParams2).then((res) => {
            if(res.data.code === 200 && res.data.message === 'success'){
                ...
                if(res.data.data.minor.length > 0){
                  this.totalPageSize2 = res.data.data.minorTotal
    ...
    searchLift(key){
      ...
      if(this.getLIftDataSecond.length > 0){
        ...
        this.getLIftDataSecond = this.mergeArrayList(obj)
        console.log("this.getLIftDataSecond===" + JSON.stringify(this.getLIftDataSecond))
      }
      
    },
    clearCode(){
      this.getLIftDataSecond = this.getLIftDataFirstOrigin
    }
